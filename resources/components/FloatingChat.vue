<template>
  <div class="wanda-floating-chat-wrapper">
    <div
    ...
    <div class="wanda-floating-window" :class="{ show: open }" v-show="open && showPopup">
      <div class="wanda-chat-header">
        ...
        <div class="wanda-header-actions">
          <button
            v-if="conversationMemoryEnabled && messages.length > 0"
            class="wanda-clear-btn"
            @click="clearConversation"
            :aria-label="msg('wanda-clear-conversation-aria')"
            :title="msg('wanda-clear-conversation')"
          ><cdx-icon :icon="cdxIconTrash" size="small"></cdx-icon></button>
      ...
      <div class="chat-container wanda-floating-chat">
        <div class="chat-box" ref="chatBox" v-show="messages.length > 0">
          <div v-for="(m, idx) in messages" :key="idx" :class="['chat-message-wrapper', m.role + '-wrapper']">
            <div class="chat-message-container">
              <div class="chat-message" :class="[ m.role + '-message' ]">
                <div v-html="m.content"></div><br>
              ...
              <!-- Display attached images below user messages -->
          ...
          <div v-if="canEdit" class="wanda-edit-card">
            ...
            <div class="wanda-edit-mode-row">
              <cdx-checkbox
                ...
                id="wanda-floating-toggle-edit-mode"
            ...
            <div v-if="editModeEnabled" class="wanda-edit-target-row">
              <label
                ...
                for="wanda-floating-edit-target-input"
              >{{ msg('wanda-edit-target-label') }}</label>
              <cdx-lookup
                id="wanda-floating-edit-target-input"
...
module.exports = exports = {
  ...
  methods: {
    ...
    onDocumentClick( e ) {
      ...
      // Don't close if image picker is open
      if ( this.imagePickerOpen ) {
        return;
      }
      // Codex menus and chip inputs teleport outside .wanda-floating-window — treat them as inside
      const insideSelector = '.wanda-floating-window, .wanda-floating-button, .wanda-image-picker-dialog, .cdx-dialog, .cdx-menu, .cdx-chip-input';
      let within = false;

      if ( typeof e.composedPath === 'function' ) {
        within = e.composedPath().some( ( node ) => node && node.matches && node.matches( insideSelector ) );
      }

      if ( !within && e.target && e.target.closest ) {
        within = !!e.target.closest( insideSelector );
      }
  ...
  formatBotResponse( rawText ) {
    ...
    saveConversationState() {
      try {
        sessionStorage.setItem( 'wanda-floating-conversation-history', JSON.stringify( this.conversationHistory ) );
        sessionStorage.setItem( 'wanda-floating-conversation-messages', JSON.stringify( this.messages ) );
        sessionStorage.setItem( 'wanda-floating-conversation-images', JSON.stringify( this.conversationImages ) );
      } catch ( e ) {
    ...
    onMemoryToggleChange() {
      try {
        sessionStorage.setItem( 'wanda-floating-memory-enabled', String( this.conversationMemoryEnabled ) );
      } catch ( e ) {
        // Ignore storage errors
      }
      if ( !this.conversationMemoryEnabled ) {
